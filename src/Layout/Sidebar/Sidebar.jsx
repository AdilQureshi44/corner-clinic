...
import {
    IconDashboard, IconFile, IconChartBar, IconBrandAppstore,
} from "@tabler/icons-react";

const Sidebar = ({isSidebarCollapsed}) => {
    const listItem = [{name: "Dashboard", icon: <IconDashboard/>}, {name: "Pages", icon: <IconFile/>}, {
        name: "Charts", icon: <IconChartBar/>
    },];

    const [activeIndex, setActiveIndex] = useState(0);

    return (<div className={`sidebar ${isSidebarCollapsed ? "collapsed" : ""}`}>
        <div className="logo">
        ...
        <div className={`sidebar-item ${isSidebarCollapsed ? "collapsed" : ""}`}>
            {listItem.map((item, index) => (<div
                ...
                <div
                    className={`sidebar-icon-name ${isSidebarCollapsed ? "collapsed" : ""}`}>
                    {item.name}
                </div>
            </div>))}
        </div>
